...
{	
	override protected void GetGestureItems(out ref array<ref GestureMenuItem> gesture_items, GestureCategories category)
	{
		...
		if (category == GestureCategories.CATEGORIES)
		{
			...
			gesture_items.Insert(new GestureMenuItem(CW95GestureCategories.CATEGORY_6, 	"Stance", category));
		}
		else if (category == GestureCategories.CATEGORY_2)
		{
			//gesture_items.Insert(new GestureMenuItem(EmoteConstants.ID_EMOTE_WHISTLE, 	"Whistle", category));
		}
		...
		else if (category == CW95GestureCategories.CATEGORY_6)
		{
...
enum CW95GestureCategories : GestureCategories
{
	CATEGORY_6
};