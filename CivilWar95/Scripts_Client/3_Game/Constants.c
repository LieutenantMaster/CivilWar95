/**
 * Constants.c
 *
 * © 2023 CivilWar95
 * By LieutenantMaster
 *
 * This work is licensed under the Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License.
 * To view a copy of this license, visit http://creativecommons.org/licenses/by-nc-nd/4.0/.
 *
*/

static const string CW95_PATH_BASE = "CivilWar95\\CivilWar95\\";
static const string CW95_PATH_GUI = CW95_PATH_BASE + "GUI\\";
static const string CW95_PATH_SCRIPTS = CW95_PATH_BASE + "Scripts_Client\\";

static const string CW95_PATH_MISSION_BASE = "$mission:cw95\\";
static const string CW95_PATH_MISSION_PLAYERDATA = CW95_PATH_MISSION_BASE + "playerdata\\";
static const string CW95_PATH_MISSION_FACTIONDATA = CW95_PATH_MISSION_BASE + "factiondata\\";

static const string CW95_PATH_PROFILE_BASE = "$profile:CW95\\";
static const string CW95_PATH_PROFILE_DATA = CW95_PATH_PROFILE_BASE + "Data\\";
static const string CW95_PATH_PROFILE_SETTINGS = CW95_PATH_PROFILE_BASE + "Settings\\";
static const string CW95_PATH_SETTINGS_SHARED = CW95_PATH_PROFILE_SETTINGS + "SharedSettings.json";
static const string CW95_PATH_SETTINGS_SERVER = CW95_PATH_PROFILE_SETTINGS + "ServerSettings.json";

static const string CW95_AS_SOUNDSET_IN = "_aim_in_SoundSet";
static const string CW95_AS_SOUNDSET_OUT = "_aim_out_SoundSet";
static const string CW95_AS_CONFIG_ENTRY = "aimSoundSet";
static const string CW95_AS_DEFAULT_SOUNDSET = "AK";