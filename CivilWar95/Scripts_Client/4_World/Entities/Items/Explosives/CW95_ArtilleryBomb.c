class CW95_ArtilleryBomb: Grenade_Base
{
	...
	static void DamageNearbyObjects(vector position)
	{
		...
		GetGame().GetObjectsAtPosition(position, 25, objectsNearBomb, null);
		for (int x = 0; x < objectsNearBomb.Count(); x++)
		{
			EntityAI entity = EntityAI.Cast(objectsNearBomb[x]);
			if (entity)
			{
				//! TODO: Use generic is under roof function from base game instead
				//if (PlayerBase.PosIsUnderRoof(entity.GetPosition() + "0 10 0"))
				//	return;

				float damageToApply = Math.RandomFloatInclusive(5, 30);
				...
				if (player)
				{
					...
					if (car)
					{
						//! TODO: Make new logic to damage automatically attachments
					}

					entity.SetHealth(entity.GetHealth() - damageToApply);
				}
			}
		}
	}
