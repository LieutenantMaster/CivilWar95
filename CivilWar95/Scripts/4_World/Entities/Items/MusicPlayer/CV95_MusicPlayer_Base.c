...
class CV95_MusicPlayer: Inventory_Base
{
	...
	CV95_Cassette_Base m_Cassette;				// The casstte hold the music data in it's rv config and store in scripts where we stopped in the music
	...
	void UpdateVisual()
	{
		...
		if ( FindAttachmentBySlotName("CV95_Cassette"))
		{
			SetAnimationPhase( "CV95_Cassette", 0 );
		}
		else
		{
			SetAnimationPhase( "CV95_Cassette", 1 );
		}
	}
	...
	void PlayButtonSound()
	{
		...
		{
			EffectSound sound =	SEffectManager.PlaySound( "CV95_Cassette_Button", GetPosition() );
	...
	void PlayDamagedTapeSound()
	{
		...
		if (health > 1)
		{
			PlaySoundSetLoop( DamagedTrack, "CV95_Ruined_Music", 0.1, 0.3 );
	...
	void PlayDoorSound(bool state)
	{
		...
		if (state)
		{
			musicSoundSet = "CV95_MusicPlayer_DoorOpen";
		}
		else
		{
			musicSoundSet = "CV95_MusicPlayer_DoorClose";
		}
	...
	{
		return ItemBase.Cast(FindAttachmentBySlotName("CV95_Cassette"));
	}
	...
	{
		m_Cassette = CV95_Cassette_Base.Cast(FindAttachmentBySlotName("CV95_Cassette"));
		...
		if(m_Cassette)
		{
			...
			//if (!GetGame().IsDedicatedServer()) 
			{
				...
				if (!GetGame().IsDedicatedServer()) 
					Track = SEffectManager.CV95_PlaySoundOnObjectWOPlay(musicsounds[0], this);
	...
	void TapeTimer(bool offset, float trackTime)
	{
		if ( !GetGame().IsDedicatedServer() )
		{
			...
			if ( offset )
			{
				Track.CV95_GetWave().GetLength();	// needs to be here otherwise it will break
				Track.CV95_SkipTo(trackTime);
				Track.SetSoundVolume(m_Volume);		// this shit does not work
				Track.CV95_GetWave().Play();
			}
		}
	...
	void StartRewinding()
	{
		...
		if ( !GetGame().IsDedicatedServer())
			RewindTrack = SEffectManager.CV95_PlaySoundOnObjectWOPlay("CV95_Cassette_Rewind", this, 0, 0, true);

		GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(DelayRewindStartSound, 50, false);
		
		if(!m_Cassette)
			m_Cassette = CV95_Cassette_Base.Cast( GetCassette() );
	...
	void DelayRewindStartSound()
	{
		...
		if(RewindTrack)
		{
			...
			RewindTrack.CV95_GetWave().GetLength();
			RewindTrack.CV95_GetWave().Play();
		}
	}
	...
	void StartFastForwarding()
	{
		...
		if ( !GetGame().IsDedicatedServer())
			FastForwardTrack = SEffectManager.CV95_PlaySoundOnObjectWOPlay("CV95_Cassette_FastForward", this, 0, 0, true);
	...
	void DelayFastForwardStartSound()
	{
		...
		if(FastForwardTrack)
		{
			...
			FastForwardTrack.CV95_GetWave().GetLength();
			FastForwardTrack.CV95_GetWave().Play();
		}
	}
